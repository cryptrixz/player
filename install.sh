...
.wave-bar {
    ...
    height: 6px;
    border-radius: 2px;
    transition: height 0.12s ease-in-out, background 0.3s ease;
    background: linear-gradient(180deg, #ff007f, #7f00ff, #00f0ff);
}
.drawer{
    -webkit-app-region: no-drag;
...
.drawer.open{
    height:240px;border:1px solid rgba(255,255,255,.22);opacity:1;padding:12px
}
.search-box{
...
let currentGradient = "linear-gradient(180deg, #ff007f, #7f00ff, #00f0ff)";
...
function updateWaveColors(r, g, b) {
    ...
    const r3 = Math.max(0, Math.floor(r * 0.6));
    const g3 = Math.max(0, Math.floor(g * 0.7));
    const b3 = Math.min(255, b + 80);
    currentGradient = `linear-gradient(180deg, rgb(${r},${g},${b}), rgb(${r2},${g2},${b2}), rgb(${r3},${g3},${b3}))`;
    document.querySelectorAll('.wave-bar').forEach(bar => {
        bar.style.background = currentGradient;
    });
}
...
function updateWindowBounds() {
    ...
    let targetH = Math.floor((drawerOpen ? 360 : 110) * baseScale);
...
function populateList(items) {
    ...
    if (!items || items.length === 0) {
        container.innerHTML = `<div class="search-hint">No items found</div>`;
        return;
    }
    items.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <div class="list-info">
                ...
                <div class="list-sub">${item.artist || ""}</div>
            </div>
            ${playMarkup}
        `;
...
document.getElementById('tabSearch').addEventListener('click', (e) => {
    ...
    document.getElementById('backBtn').style.display = 'none';
    document.getElementById('drawerTabs').style.display = 'flex';
    document.getElementById('listContainer').innerHTML = `<div class="search-hint">Type in the search box above and press Enter to play</div>`;
